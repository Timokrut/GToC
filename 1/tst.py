...
def find_amount_of_signals(V_inf: int, T: float) -> float:
    log_q = T * V_inf
    return 2 ** log_q 
...
def calculate_signal(S_i: tuple[float, float], T: float, f_0: int) -> tuple[list[float], list[float]]:
    Si_1, Si_2 = S_i 
    x = []
    y = []
...
def count_si_wo_A(i_pair: tuple, q: float) -> tuple:
    i_1, i_2 = i_pair
...
def count_energy_of_signal(S_i: tuple[float, float]):
    Si_1, Si_2 = S_i 
